export const getMessagesByError = (error: {
  code?: string;
  message: string;
}) => {
  if (!error.code) {
  ...
  switch (error.code) {
    ...
    case 'auth/code-expired':
      return 'O código SMS expirou. Reenvie o código de verificação para tentar novamente.';
    ...
    case 'storage/invalid-event-name':
      return 'Nome inválido do evento fornecido. Precisa ser um dos valores [`running`, `progress`, `pause`]';
    case 'storage/invalid-url':
      return 'URL inválido fornecido para refFromURL(). Deve estar no formato: gs://bucket/object ou https://firebasestorage.googleapis.com/v0/b/bucket/o/object?token=&ltTOKEN>.';
    case 'storage/invalid-argument':
      return 'O argumento transmitido a put() precisa ser matriz "File", "Blob" ou "UInt8". O argumento transmitido a putString() precisa ser uma string bruta "Base64" ou "Base64URL".';
    ...
    case 'invalid-argument':
      return 'Foi especificado um argumento inválido, por exemplo, um nome de campo inválido';
    case 'deadline-exceeded':
      return 'O prazo expirou antes que a operação pudesse ser concluída.';
    case 'not-found':
      return 'Documento solicitado não foi encontrado.';
    case 'already-exists':
      return 'O documento que está tentando criar já existe';
    case 'permission-denied':
      return 'Sem permissões para executar a operação especificada.';
    case 'resource-exhausted':
      return 'Algum recurso foi esgotado, talvez uma cota por usuário ou talvez todo o sistema de arquivos esteja sem espaço.';
    case 'failed-precondition':
      return 'A operação foi rejeitada porque o sistema não está em um estado necessário para a execução da operação.';
    case 'aborted':
      return 'A operação foi cancelada.';
    case 'out-of-range':
      return 'A operação foi tentada além do intervalo válido.';
    case 'unimplemented':
      return 'A operação não foi implementada ou não é suportada';
    case 'internal':
      return 'Ocorreu um erro interno ao tentar realizar a operação.';
    case 'unavalable':
